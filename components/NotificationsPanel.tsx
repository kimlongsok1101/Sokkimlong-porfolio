      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 h-screen w-full max-w-md bg-slate-950 border-l border-slate-800 z-50 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between gap-4 p-6 border-b border-slate-800/80">
                  ...
                  {notifications.map((notification) => {
                    ...
                    const NotificationContent = (
                      <motion.div
                        ...
                        exit={{ opacity: 0, x: 20 }}
                        className={`p-4 hover:bg-slate-900/50 transition-colors cursor-pointer ${
                          !notification.read ? "bg-indigo-500/5" : ""
                        }`}
                      >
                        {/* Project Image Preview */}
                        {imageSrc && (
                          <div className="relative w-full h-32 rounded-lg overflow-hidden mb-3 bg-slate-900 border border-slate-800">
                            <Image
                              src={imageSrc}
            ...
            {!loading && notifications.length > 0 && unreadCount > 0 && (
              <div className="border-t border-slate-800/80 p-4">
                <button
                  ...
                  className="w-full py-2.5 rounded-lg bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 text-indigo-300 text-sm font-medium transition-colors flex items-center justify-center gap-2"
                >
