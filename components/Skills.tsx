...
import { Sparkles, Layout, Palette, Cpu, Terminal, Database } from "lucide-react";
...
// Inline icon components and resolver (moved here per admin request)
import type { SVGProps } from "react";

const wrapClass = (props: SVGProps<SVGSVGElement>, defaultClass: string) =>
  props.className ? props.className : defaultClass;

export const ReactIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} className={wrapClass(props, "w-6 h-6 text-sky-400")} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
...
export const NextjsIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} className={wrapClass(props, "w-6 h-6 text-white")} viewBox="0 0 24 24" fill="currentColor">
...
export const TypescriptIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} className={wrapClass(props, "w-6 h-6 text-blue-400")} viewBox="0 0 24 24" fill="currentColor">
...
export const TailwindIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} className={wrapClass(props, "w-6 h-6 text-cyan-400")} viewBox="0 0 24 24" fill="currentColor">
...
export const NodejsIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} className={wrapClass(props, "w-6 h-6 text-emerald-400")} viewBox="0 0 24 24" fill="currentColor">
...
export const PostgresIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} className={wrapClass(props, "w-6 h-6 text-sky-500")} viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 16h-2v-2h2v2zm1.07-7.75l-.9.92C12.45 11.9 12 12.5 12 14h-2v-.5c0-1.1.45-2.1 1.17-2.83l1.24-1.26c.37-.36.59-.86.59-1.41 0-1.1-.9-2-2-2s-2 .9-2 2H7c0-2.76 2.24-5 5-5s5 2.24 5 5c0 1.04-.42 1.99-1.07 2.75z" />
  </svg>
);

export const GitIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} className={wrapClass(props, "w-6 h-6 text-orange-500")} viewBox="0 0 24 24" fill="currentColor">
...
export const PhotoshopIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} className={wrapClass(props, "w-6 h-6 text-sky-400")} viewBox="0 0 24 24" fill="currentColor">
...
export const IllustratorIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} className={wrapClass(props, "w-6 h-6 text-amber-500")} viewBox="0 0 24 24" fill="currentColor">
...
export const FigmaIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} className={wrapClass(props, "w-6 h-6 text-pink-400")} viewBox="0 0 24 24" fill="currentColor">
...
export const DefaultSkillIcon = (props?: { className?: string }) => {
  const { className, ...rest } = props || {};
  const cls = className ?? "w-6 h-6 rounded-xl bg-slate-800 flex items-center justify-center text-slate-400 text-[10px]";
  return (
...
export function resolveIcon(icon?: string | ((props?: any) => any)) {
  if (!icon) return DefaultSkillIcon;
  if (typeof icon === "function") return icon;
  const orig = icon as string;
  if (ICON_MAP[orig]) return ICON_MAP[orig];
  let key = normalizeKey(orig);
  if (key.endsWith("icon")) key = key.slice(0, -4);
  if (VARIANTS[key] && ICON_MAP[VARIANTS[key]]) return ICON_MAP[VARIANTS[key]];
  for (const k of Object.keys(ICON_MAP)) {
    if (normalizeKey(k) === key) return ICON_MAP[k];
  }
  for (const k of Object.keys(ICON_MAP)) {
    if (normalizeKey(k).includes(key) || key.includes(normalizeKey(k))) return ICON_MAP[k];
  }
